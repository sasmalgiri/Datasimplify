 ...
 * - 'native_charts': CRK formulas + chart-ready data layout with prefetched data (default)
 * - 'formulas_only': Only CRK formulas, no charts
 */
export type ContentType = 'formulas_only' | 'native_charts';
...
export interface UserTemplateConfig {
  ...
  contentType?: ContentType; // Type of content to generate (default: 'native_charts')
  formulaMode?: FormulaMode; // 'crk' only - BYOK with prefetched data
  customizations: {
 ...
 * - 'native_charts': Formulas + chart-ready data layout with prefetched data (default)
...
export async function generateTemplate(
  ...
  // Determine content type (default to 'native_charts' for best experience)
  const contentType = userConfig.contentType || 'native_charts';
  ...
  const branding = 'CRK BYOK (Prefetched)';
  workbook.description = `${baseTemplate.description} - Data via ${branding}`;
...
async function createCRKMetadataSheet(
  ...
  // Last refresh timestamp (set at generation time with prefetched data)
  sheet.getCell(`A${row}`).value = 'LAST_REFRESH';
  sheet.getCell(`B${row}`).value = '';
  row += 1;

  // Refresh count (tracks data refreshes)
  ...
  const attribution = [
    ...
    userConfig.formulaMode === 'crk'
      ...
      : 'Data via CRK (cryptoreportkit.com)',
  ...
  const instructions = [
    ...
    '1. Data tab - Click "Refresh All" or press Ctrl+Alt+F9',
    '2. Web dashboards - Visit cryptoreportkit.com for live interactive data',
...
async function createSetupSheet(
  ...
  subtitleCell.value = 'CryptoReportKit Template - Data via BYOK (Prefetched)';
  ...
  const requirements = [
    ...
    ['✓ Data Prefetched', 'Template ships with current data ready to use'],
    ['✓ Live Dashboards', 'Visit cryptoreportkit.com for live updates with BYOK'],
  ];
  ...
  const statusFormula = 'IFERROR(CRK.INFO("bitcoin","name"), "Prefetched data loaded")';

  sheet.getCell(`B${row}`).value = statusLabel;
  sheet.getCell(`B${row}`).font = { color: { argb: COLORS.textSecondary } };
  sheet.getCell(`B${row}`).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: COLORS.bgMedium } };
  ...
  const issues = [
    ['#NAME? errors', 'Formula references not available → Data is prefetched in this template'],
    ['#VALUE! errors', 'Invalid parameter → Check cell references'],
  ...
  const links = [
    ['Live Dashboards', 'https://cryptoreportkit.com/live-dashboards'],
...
async function createDataSheet(
  ...
  const isCRK = userConfig.formulaMode === 'crk';
  const dataSource = 'Data via BYOK (Prefetched)';
...
// Only native_charts and formulas_only content types are available

/**
 * Create template usage instructions sheet
 * Guides users on working with prefetched data and web dashboards
 */
async function createAddinInstructionsSheet(
  ...
  sheet.mergeCells(`B${row}:C${row}`);
  const titleCell = sheet.getCell(`B${row}`);
  titleCell.value = '📊 Interactive Charts with CryptoReportKit';
  ...
  const requirements = [
    ...
    ['Prefetched Data', 'Template ships with current data ready to use'],
    ['Web Dashboards', 'Visit cryptoreportkit.com for live interactive data'],
    ['Internet connection', 'Required for live web dashboards only'],
  ];
  ...
  const steps = [
    ['Step 1:', 'Open this file in Microsoft Excel Desktop'],
    ['Step 2:', 'Data is already prefetched and ready to view'],
    ['Step 3:', 'Go to Data tab to explore the data sheets'],
    ['Step 4:', 'Select data range and Insert → Chart for visualizations'],
    ['Step 5:', 'For live data, visit cryptoreportkit.com/live-dashboards'],
    ['Step 6:', 'Connect your CoinGecko API key (BYOK) on the web'],
    ['Step 7:', 'Download updated templates anytime'],
    ['Step 8:', 'Enjoy your data!'],
  ];
  ...
  footerCell.value = '💡 The Data sheet contains your prefetched data. Visit cryptoreportkit.com for live interactive dashboards.';
...
async function createNativeChartsSheet(
  ...
  sheet.mergeCells(`B${row}:C${row}`);
  const footerCell = sheet.getCell(`B${row}`);
  footerCell.value = '💡 Tip: Charts created this way are embedded in the Excel file and will work for anyone you share it with!';
  footerCell.font = { italic: true, color: { argb: COLORS.primary } };
  footerCell.alignment = { wrapText: true };
}
...
async function createInstructionsSheet(
  ...
  const gettingStarted = [
    ...
    ['3', 'If you see #NAME? errors → Data is prefetched, formulas may not resolve', ''],
    ['4', 'For live data, visit cryptoreportkit.com/live-dashboards', 'BYOK - use your own API key'],
  ...
  const troubleshooting = [
    ['#NAME? error', 'Formula references not available — data is prefetched in this template'],
