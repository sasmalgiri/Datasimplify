...
export function CorrelationHeatmap({ coins, showBeginnerTips = true }: CorrelationHeatmapProps) {
  ...
  const getColor = (value: number) => {
    ...
    if (value >= 0.3) return 'bg-green-300 text-gray-900';
    if (value >= 0.1) return 'bg-green-200 text-gray-900';
    if (value >= -0.1) return 'bg-gray-400 text-gray-900';
    if (value >= -0.3) return 'bg-red-200 text-gray-900';
    if (value >= -0.5) return 'bg-red-300 text-gray-900';
    ...
    <div className="bg-gray-800/60 rounded-xl border border-gray-700/50 p-6 text-white">
          ...
          <div className="flex">
            ...
            {coins.map(coin => (
              <div 
                ...
                className="w-16 h-12 flex items-center justify-center text-xs font-bold text-gray-400"
              >
      ...
      {selectedCell && selectedCell.coin1 !== selectedCell.coin2 && (
        <div className="mt-6 p-4 bg-gray-700/50 rounded-lg border border-gray-700/50">
          ...
          <div className="mt-4 space-y-3">
            <div>
              <p className="text-sm text-gray-400">Correlation Level</p>
              <p className="font-medium">{getInterpretation(selectedCell.value).level}</p>
            </div>
            <div>
              <p className="text-sm text-gray-400">What This Means</p>
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-lg">
        ...
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-gray-400 mb-1"><strong>Best Pairs for Diversification:</strong></p>
            <p className="text-gray-300">
              Look for cells with values close to 0 or negative. These coins don&apos;t move together!
            </p>
          </div>
          <div>
            <p className="text-gray-400 mb-1"><strong>Avoid Redundancy:</strong></p>
            <p className="text-gray-300">
    ...
    return (
      <div className="bg-gray-800/60 rounded-xl border border-gray-700/50 p-6">
        <div className="flex items-center justify-center py-8">
          ...
          <span className="ml-3 text-gray-400">Loading correlation data...</span>
        </div>
      </div>
    );
  }
