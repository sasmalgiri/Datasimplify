 ...
 *
 * Run with: npx ts-node scripts/backfill-historical-data.ts
 */
...
async function backfillFearGreed() {
  ...
  try {
    ...
    const records = json.data.map((item: { value: string; value_classification: string; timestamp: string }) => ({
...
async function backfillPriceHistory() {
  ...
  for (const coin of coins) {
    try {
      ...
      if (!response.ok) {
        console.log(`  ⚠️ ${coin}: Rate limited, skipping`);
        continue;
      }
      ...
      const records = json.prices.map((price: [number, number], index: number) => ({
        symbol: coin.toUpperCase(),
        interval: '1d',
        open_time: new Date(price[0]).toISOString(),
        close: price[1],
        open: price[1], // Approximate
